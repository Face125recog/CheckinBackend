...
def img2vector(fileNamestr):
    ...
    img = cv2.resize(image, (100, 100))  # 调整图片大小
    img = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)  # 调整为灰度图
    img = img.reshape((1, 1, 100, 100))
    #img = np.asarray(image).reshape(100, 100)  # 将灰度图转化为对应像素大小的数组
    # image = Image.open(fileNamestr).convert('L')  # 以灰度图像形式打开
    # img = np.asarray(image).reshape(100, 100)  # 将灰度图转化为对应像素大小的数组
    return img
